...
export function Projects() {
  ...
  const [imageSettings, setImageSettings] = useState<ProjectImageSettings>({})

  useEffect(() => {
    let cancelled = false
    loadProjectImageSettings().then((settings) => {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      cards.forEach((card, i) => {
        ...
        const imageLayer = card.querySelector('[data-project-image-layer]')
        if (imageLayer) {
          gsap.fromTo(
            imageLayer,
            { yPercent: -3 },
            { yPercent: 3, ease: 'none', scrollTrigger: { trigger: card, start: 'top bottom', end: 'bottom top', scrub: .6 } },
          )
        }
      })
    }, sectionRef)
  ...
  return (
    <section id="projects" ref={sectionRef} className="px-6 py-28 sm:py-36">
      <div className="mx-auto mb-14 max-w-lg text-center">
        <p className="mb-3 text-sm font-semibold uppercase tracking-[0.14em] text-mist">
          <span className="contact-spark mr-2 inline-block h-1.5 w-1.5 rounded-full bg-accent align-middle" aria-hidden />
          <TextShimmer>Selected Work</TextShimmer>
        </p>
      ...
      <div ref={gridRef} className="mx-auto grid max-w-5xl grid-cols-1 gap-x-6 gap-y-14 sm:grid-cols-2">
        {projects.map((p, i) => {
          const setting = imageSettings[p.id] || defaultProjectImageSetting()
          const imageScale = `scale(${setting.scale / 100})`
          const Card = (
            <div
            ...
            >
              {p.cover_image && (
                <div data-project-image-layer className="h-full w-full">
                  <img
                    src={p.cover_image}
                    alt={p.title}
                    loading="lazy"
                    className={`h-full w-full ${setting.fit === 'contain' ? 'object-contain' : 'object-cover'}`}
                    style={{
